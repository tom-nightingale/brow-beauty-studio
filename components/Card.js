...
export default function Card({ url, image, title, date }) {
  const newDate = date ? new Date(parseISO(date)) : new Date();
  ...
  return (
    <div className="w-full mb-4 sm:w-1/2 xl:w-1/4 sm:px-4">
      <Link
        href={`/${url}`}
        scroll={false}
        className="block p-6 transition-all duration-200 bg-white shadow-lg rounded-2xl md:p-8 hover:shadow-xl"
      >
        ...
        <span className="block mt-4 mb-2 text-lg tracking-widest text-center uppercase md:mt-6">
          {title}
        </span>
      </Link>
    </div>
